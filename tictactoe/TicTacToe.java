...
public class TicTacToe implements ActionListener {
	...
	TicTacToe(){
		...
		title_panel.setBounds(0,0,660,60);
			...
			if(e.getSource()==buttons[i]) {
				if(player1_turn) {
					if(buttons[i].getText()=="") {
						buttons[i].setForeground(new Color(255,0,0));//blue
						buttons[i].setText("X");
						player1_turn = false; //change the game turn
						textField.setText("O turn");
						check(); //check for winning condition
					}
				}else {
					if(buttons[i].getText()=="") {
						buttons[i].setForeground(new Color(0,0,255));//read
						buttons[i].setText("O");
						player1_turn = true;
